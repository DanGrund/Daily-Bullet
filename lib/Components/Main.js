...
export default class Main extends React.Component {
  ...
  addSubList(listKey, subTask){
    let user = (this.state.user.uid)
    firebase.database().ref(user+'/'+listKey+'/sublist/').push(subTask)
  }

  deleteSubTask(listKey, id) {
    let user = (this.state.user.uid)
    firebase.database().ref(user+'/'+listKey+'/sublist/').child(id).remove();
  }
  ...
  render() {
    ...
    return (
      <div>
        <h1>Daily Bullet.</h1>
        <h2> Welcome {this.state.user.email}   <button
